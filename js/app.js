...
(function () {
  ...
  const DEFAULT_AWARD_PLAN = {
    ...
    bothNodup: true,
    groupAward1: { enabled: false, threshold: 5, payout: 5, allowOverlap: false },
    groupAward2: { enabled: false, rateThreshold: 110, payout: 15, allowOverlap: false },
    // 4. 기준조건 (eligibility)
    eligibility: {
      enabled: true,
      operator: "and",
        ...
        ${(() => {
          ...
          const mkCard = (c) => `
            <div class="aps-award-card${c.isItem ? " aps-item-prize" : ""}${c.isTeam ? " aps-team-prize" : ""}">
              <div class="aps-awd-head">${c.icon} ${escapeHtml(c.label)}${c.allowOverlap ? `<span style="margin-left:5px;font-size:10px;background:#E3F2FD;color:#1565C0;border-radius:3px;padding:1px 4px;vertical-align:middle;">중복시상</span>` : ""}</div>
    ...
    if (plan.personalIncr?.enabled && pa.tiers.length > 0) {
      ...
      if (tierHit) {
        ...
        statusHtml = `<div class="hl-row" style="margin-bottom:4px;">
          ...
          <div class="hl-amt grn4" style="font-size:26px;">${escapeHtml(prize)}</div>
        </div>`;
      } else {
      ...
      // 전 구간 테이블 — 최고 달성 구간 하나만 up-next(노란 배경+굵게) 처리
      const tierRows = pa.tiers.map((t) => {
        const isHit = tierHit && t === tierHit;
        const prizeStr = t.type === "pct"
          ? `${t.payVal}%`
          : t.type === "item" ? (t.itemName || "물품") : `${t.payVal}만원`;
        return `<tr class="${isHit ? "up-next" : ""}">
          <td${isHit ? ' style="font-weight:900;font-size:14px;"' : ' style="font-size:14px;"'}>${fmtRaw(t.min)} 이상</td>
          <td${isHit ? ' style="font-weight:900;color:#1B5E20;font-size:14px;"' : ' style="font-size:14px;"'}>${escapeHtml(prizeStr)}</td>
        </tr>`;
      }).join("");
      piSection = `<div class="sec-title grn4">🎁 개인순증시상</div>
        ${statusHtml}
        <table class="up-table" style="margin-top:2px;font-size:14px;">
...
<style>${AWARD_PRINT_CSS}
  ...
  function renderHomeRanks() {
    ...
    hrCats.forEach(cat => {
      if (cat.key === "group") {
        ...
        const ttls = { rate: `📈 신장률 / 💰 신장액 통합 순위 (${byRateRaw.length}명)`, amt: `💰 신장액 / 📈 신장률 통합 순위 (${byAmt.length}명)`, ipum: `✨ 인품왕 전체 순위 (${byIpum.length}명)` };
        const subs = { rate: "달성률 기준 정렬 · 신장액 시상 병행 표시", amt: "순증액 기준 정렬 · 신장률 시상 병행 표시", ipum: "신상품 판매액" };
        let body;
        if ((cat.key === "rate" || cat.key === "amt") && _hrankPa.bothEnabled) {
          // 신장률·신장액 둘 다 활성: 통합 뷰 (카드 종류에 따라 정렬 기준 선택)
          const hasList = cat.key === "amt" ? byAmt.length : byRateRaw.length;
          body = hasList ? renderProgressRankCombined(byRateRaw, byAmt, _hrankPa, cat.key) : `<div class="pg-empty">데이터 없음</div>`;
        } else if (cat.key === "rate") {
  ...
  // 신장률+신장액 통합 순위표 — 달성률 기준 정렬, 양쪽 시상 열 나란히 표시
  function renderProgressRankCombined(byRate, byAmt, pa, sortBy) {
    sortBy = sortBy || "rate";
    const iterList = sortBy === "amt" ? byAmt : byRate;
    const { rateAsgn, amtAsgn } = computeBothAwardAssignments(byRate, byAmt, pa);
    const ratePayouts = pa.rateConfig?.payouts || [];
    const amtPayouts  = pa.amtConfig?.payouts  || [];
    ...
    const rows = iterList.map((st) => {
      const empNo  = st.s.empNo;
      const rateVal = (st.rate != null ? st.rate.toFixed(1) : "0.0") + "%";
      let rowCls = "";

      // ── 신장률 시상 셀 ──
      const ra = rateAsgn.get(empNo) || { status: "none" };
      let rPrizeCell;
      if (ra.status === "mine") {
        rPrizeCell = `<span class="pg-bdg pg-b-g">${ra.effectiveRank}위 ${escapeHtml(prizeLabel(ra.effectiveRank, ratePayouts, ra.effectiveAmt))}</span>`;
      } else if (ra.status === "other") {
        const aa2 = amtAsgn.get(empNo);
        const aPrize = aa2?.effectiveRank > 0 ? escapeHtml(prizeLabel(aa2.effectiveRank, amtPayouts, aa2.effectiveAmt || 0)) : "";
        rPrizeCell = `<span class="pg-bdg pg-rank-swap">💰 신장액${aa2?.effectiveRank > 0 ? ` ${aa2.effectiveRank}위` : ""}${aPrize ? ` ${aPrize}` : ""} 시상</span>`;
        rowCls     = " pg-rank-deferred";
      } else if (ra.status === "ineligible") {
        rPrizeCell = `<span class="pg-bdg pg-b-no">${ra.needsNet > 0 ? `+${Nf(ra.needsNet)}원 필요` : "기준미달"}</span>`;
      } else {
        rPrizeCell = `<span style="color:#bbb">-</span>`;
      }

      // ── 신장액 시상 셀 ──
      const aa = amtAsgn.get(empNo) || { status: "none" };
      let aPrizeCell;
      if (aa.status === "mine") {
        aPrizeCell = `<span class="pg-bdg pg-b-g">${aa.effectiveRank}위 ${escapeHtml(prizeLabel(aa.effectiveRank, amtPayouts, aa.effectiveAmt))}</span>`;
      } else if (aa.status === "other") {
        aPrizeCell = `<span style="color:#94a3b8;font-size:11px">신장률 수상</span>`;
      } else if (aa.status === "ineligible") {
        aPrizeCell = `<span class="pg-bdg pg-b-no">${aa.needsNet > 0 ? `+${Nf(aa.needsNet)}원 필요` : "기준미달"}</span>`;
      } else {
        aPrizeCell = `<span style="color:#bbb">-</span>`;
      }

      // 순증(원): minNet 미달시 붉은색
      const isNetLow = ra.needsNet > 0 || aa.needsNet > 0;
      const netStr = (st.net >= 0 ? "+" : "") + Nf(st.net) + "원";
      const netVal = isNetLow ? `<span style="color:#ef4444;font-weight:600">${netStr}</span>` : netStr;

      return `<tr class="pg-tr-click${rowCls}" data-emp="${escapeHtml(empNo)}">
        <td><strong>${escapeHtml(st.s.name || "")}</strong></td><td>${escapeHtml(st.s.branch || "")}</td><td class="r">${rateVal}</td><td class="r">${netVal}</td><td>${rPrizeCell}</td><td>${aPrizeCell}</td>
      </tr>`;
    }).join("");

    const noticeText = sortBy === "amt"
      ? "💰 신장액 기준 정렬 · 신장률 시상은 좌측 열에 표시 (중복시상 없음 — 더 큰 시상 1회만 지급)"
      : "📈 신장률 기준 정렬 · 신장액 시상은 우측 열에 표시 (중복시상 없음 — 더 큰 시상 1회만 지급)";

    return `<div class="pg-rank-notice" style="background:#f0fdf4;border-color:#86efac;">${noticeText}</div>
      <table class="pg-tbl">
        <thead><tr><th>성명</th><th>지점</th><th>달성률</th><th>순증(원)</th><th>신장률 시상</th><th>신장액 시상</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>`;
  }
  ...
  function openProgressPrintWindow() {
    ...
    // 3개: 상단 2단(신장률|신장액) + 하단 전체(팀시상) — CSS가 grid-column 제어
    const awardGridCols = sectCount >= 2 ? "1fr 1fr" : "1fr";
    ...
    const piRowsArr = byAmt.map((st, i) => {
      ...
      // 개인시상: 달성자 → 시상금액 + 상위 단계 부족액 / 미달성자 → 최초 시상까지 부족액
      const tiers = _pa.tiers || [];
      const ctIdx = tiers.findIndex(t => st.net >= t.min);
      let awCol;
      if (ctIdx >= 0) {
        const ct      = tiers[ctIdx];
        const awLabel = tierLabel(st.net, region, _pa);
        const prize   = ct.type === "pct" ? Math.round(st.net * ct.val) : ct.type === "item" ? 0 : ct.val;
        const pStr    = ct.type === "item" ? (ct.itemName || "물품") : prize > 0 ? Nf(prize) + "원" : "";
        const nt      = ctIdx > 0 ? tiers[ctIdx - 1] : null;
        const nStr    = nt ? "↑" + Nf(nt.min - st.net) + "원" : "🏆최고";
        awCol = `<span style="color:#166534;font-weight:700;">${escapeHtml(awLabel)}</span>` +
          (pStr ? `<br><span style="color:#1d4ed8;font-size:0.85em;">(${pStr})</span>` : "") +
          `<br><span style="color:#b45309;font-size:0.8em;">${nStr}</span>`;
      } else {
        const lt   = tiers.length > 0 ? tiers[tiers.length - 1] : null;
        const need = lt ? Math.max(0, lt.min - st.net) : 0;
        awCol = need > 0
          ? `<span style="color:#94a3b8;font-size:0.9em;">↑${Nf(need)}원</span>`
  ...
  <table style="width:100%;border-collapse:collapse;font-size:15px;">
    ...
    // ── 인쇄 스타일: body.pg-printing 클래스 기반 (position:fixed 블랭크 버그 회피) ──
    const stel = document.createElement("style");
    stel.id = "pg-print-style";
    stel.textContent = `
#pg-print-overlay td.r { text-align:right; }
...
#pg-print-overlay .pg-grp-miss { color:#9ca3af;font-size:11px; }
...
@media print {
  ...
  /* ── 시상 페이지 공통 ── */
  #pp-page1 { padding:4mm 8mm 3mm!important; }
  ...
  #pp-page1 .pg-rank-notice { font-size:9px!important; padding:1px 4px!important; margin-bottom:2px!important; }
  #pp-page1 .pg-tbl { width:100%!important; table-layout:auto!important; border-collapse:collapse!important; }
  #pp-page1 .pg-tbl th, #pp-page1 .pg-tbl td { white-space:nowrap!important; overflow:visible!important; }

  /* 1개 섹션: 그리드 해제 → 블록으로 100% 너비 꽉 채움, 자동 폰트 스케일 */
  #pp-page1.pp-sect1 .award-grid { display:block!important; width:100%!important; }
  #pp-page1.pp-sect1 .award-grid > div { width:100%!important; }
  #pp-page1.pp-sect1 h3 { font-size:14px!important; padding:5px 10px!important; margin-bottom:6px!important; }
  #pp-page1.pp-sect1 .pg-tbl th, #pp-page1.pp-sect1 .pg-tbl td { font-size:14px!important; padding:5px 10px!important; }

  /* 2개 섹션: 2단 나란히, 중간 폰트 */
  #pp-page1.pp-sect2 .award-grid { grid-template-columns:1fr 1fr!important; }
  #pp-page1.pp-sect2 h3 { font-size:11px!important; padding:3px 7px!important; margin-bottom:4px!important; }
  #pp-page1.pp-sect2 .pg-tbl th, #pp-page1.pp-sect2 .pg-tbl td { font-size:11px!important; padding:3px 5px!important; }

  /* 3개 섹션: 상단 2단(신장률|신장액) + 하단 전체너비(팀시상) */
  #pp-page1.pp-sect3 .award-grid { grid-template-columns:1fr 1fr!important; }
  #pp-page1.pp-sect3 #pp-sec-grp { grid-column:1 / -1!important; margin-top:6px!important; }
  ...
  .pp-page-pi th, .pp-page-pi td { padding:3px 5px!important; font-size:14px!important; white-space:nowrap!important; overflow:visible!important; }
    ...
    overlay.style.cssText = "position:fixed;inset:0;z-index:9999;background:#fff;overflow-y:auto;font-family:'Apple SD Gothic Neo','Malgun Gothic',sans-serif;font-size:14px;color:#111;";
    overlay.innerHTML = `
<div id="pg-print-ctrl" style="position:sticky;top:0;z-index:1000;background:#1e293b;color:#fff;padding:9px 16px;">
  ...
  <div style="display:flex;gap:14px;flex-wrap:wrap;margin-top:8px;padding-top:8px;border-top:1px solid #334155;">
    <span style="font-size:12px;color:#94a3b8;">인쇄 항목 선택:</span>
    ${hasRate ? `<label style="${ckS}"><input type="checkbox" id="pck-rate" checked style="accent-color:#3b82f6;width:15px;height:15px;"> 📈 신장률 시상</label>` : ""}
    ${hasAmt  ? `<label style="${ckS}"><input type="checkbox" id="pck-amt"  checked style="accent-color:#3b82f6;width:15px;height:15px;"> 💰 신장액 시상</label>` : ""}
    ${hasGrp  ? `<label style="${ckS}"><input type="checkbox" id="pck-grp"  checked style="accent-color:#3b82f6;width:15px;height:15px;"> 🏅 그룹별 시상</label>` : ""}
    <label style="${ckS} opacity:0.5;cursor:default;"><input type="checkbox" checked disabled style="width:15px;height:15px;"> 📊 개인실적 (필수)</label>
  </div>
</div>

${hasRate || hasAmt || hasGrp ? `
<div class="pp-page pp-sect${sectCount}" id="pp-page1" style="padding:8mm 10mm 4mm;">
...
${piPagesHtml}`;
    ...
    function updatePrintLayout() {
      ...
      if (document.getElementById("pp-sec-rate")) document.getElementById("pp-sec-rate").style.display = r ? "" : "none";
      if (document.getElementById("pp-sec-amt"))  document.getElementById("pp-sec-amt").style.display  = a ? "" : "none";
      if (document.getElementById("pp-sec-grp"))  document.getElementById("pp-sec-grp").style.display  = g ? "" : "none";
      const p1 = document.getElementById("pp-page1");
      if (p1) p1.style.display = (r || a || g) ? "" : "none";
    }
...
1005678  2  300000"></textarea>
  ...
  function renderMasterStats() {
    const body = document.getElementById("stats-master-body");
    if (!body) return;

    // 기수 셀렉트 채우기 (중복 방지)
    const cohortSel = document.getElementById("stat-cohort-sel");
    const stepSel = document.getElementById("stat-step-sel");
    if (cohortSel) {
      const prevCohort = cohortSel.value;
      const cohorts = [...new Set(
        state.students.map((s) => String(s.cohort || "").replace(/기$/, "")).filter(Boolean)
    ...
    if (!filtered.length) {
      body.innerHTML = `<div class="empty-state">조건에 맞는 교육생이 없습니다. 기수를 변경하거나 실적진도 데이터를 먼저 입력하세요.</div>`;
      return;
    }
    ...
    const computeRegion = (rs, rObj, sfx) => {
      ...
      const cur = rs.reduce((a, s) => {
        const v = sfx ? Number(s[`pgCurrent${sfx}`] || 0) : Number(s.pgCurrent || s.current || 0);
        return a + v;
      }, 0);
      const net = cur - base;
      const rate = base > 0 ? (cur / base) * 100 : 0;
      const curAvg = count > 0 ? Math.round(cur / count) : 0;
      const ipumCount = rs.reduce((a, s) => a + Number(sfx ? (s[`pgIpumCount${sfx}`] || 0) : (s.pgIpumCount || 0)), 0);
      ...
      const awardPrize = rs.reduce((a, s) => {
        ...
        if (n >= 100000) return a + 100000;
        if (n >= 50000)  return a + 50000;
        return a;
      }, 0);
    ...
    orgRegions.forEach((r) => {
      const rs = filtered.filter((s) => s.region === r.name);
      if (!rs.length) return;
    ...
    if (!regionRows.length) {
      body.innerHTML = `<div class="empty-state">해당 기수의 실적진도 데이터가 없습니다.</div>`;
      return;
    }

    // 순위 배정 (달성률 기준)
    const byRate1 = [...regionRows].sort((a, b) => b.d1.rate - a.d1.rate);
    byRate1.forEach((r, i) => { r.d1.rank = i + 1; });
    if (step === "2") {
      const byRate2 = [...regionRows].filter((r) => r.d2).sort((a, b) => (b.d2?.rate || 0) - (a.d2?.rate || 0));
    ...
    const tot = {
      ...
      base: sumD("base"),
      cur: sumD("cur"),
      ipumCount: sumD("ipumCount"),
      ipumAmt: sumD("ipumAmt"),
      achiever5k: sumD("achiever5k"),
    ...
    tot.rate = tot.base > 0 ? (tot.cur / tot.base) * 100 : 0;
    ...
    const rateClass = (v) => v >= 100 ? "stat-rate-hi" : v >= 80 ? "stat-rate-mid" : "";
    ...
    const thead2 = `
      ...
      <tr>
        ...
        <th class="tc" style="background:#2a3d7a;">달성률</th>
        <th class="tc" style="background:#2a3d7a;">순위</th>
        <th colspan="2" class="tc" style="background:#1a5a3a;">현재실적</th>
        <th class="tc" style="background:#1a5a3a;">달성률</th>
        <th class="tc" style="background:#1a5a3a;">순위</th>
        <th class="tr" style="background:#1a5a3a;">인당</th>
        <th colspan="3" class="tc" style="background:#1a5a3a;">인생의품격</th>
    ...
    const totHtml1 = `<tr class="stat-total-row">
      ...
      <td class="tr"><strong>${fK(tot.net)}</strong></td>
      <td class="tc ${rc}"><strong>${fR(tot.rate)}</strong></td>
      <td class="tc">-</td>
      <td class="tr"><strong>${fK(tot.curAvg)}</strong></td>
      <td class="tc"><strong>${fN(tot.ipumCount)}</strong></td>
      <td class="tr"><strong>${fK(tot.ipumAmt)}</strong></td>
      <td class="tr"><strong>${tot.ipumRatio.toFixed(1)}%</strong></td>
    ...
    const chartBars = chartData.map((d) => {
      ...
      const fill = d.rate >= 100 ? "blue" : d.rate >= 80 ? "orange" : "gray";
      return `<div class="stat-chart-row">
        <div class="stat-chart-label">${escapeHtml(d.label)}</div>
        <div class="stat-chart-track"><div class="stat-chart-fill ${fill}" style="width:${pct}%"></div></div>
        <div class="stat-chart-val ${rateClass(d.rate)}">${d.rate.toFixed(1)}%</div>
    ...
    body.innerHTML = `
      <div style="font-size:14px;font-weight:800;color:var(--ink);margin-bottom:4px;">
        ${cohortLabel}${stepLabel} 현황
        <span style="font-size:11px;font-weight:400;color:var(--ink-3);margin-left:8px;">금액 단위: 천원</span>
      </div>

      <div class="stat-summary-bar">
        <div class="stat-sum-item"><div class="stat-sum-label">총 입교인원</div><strong>${fN(tot.count)}명</strong></div>
        <div class="stat-sum-item"><div class="stat-sum-label">기준실적 합계</div><strong>${fK(tot.base)}천원</strong></div>
        <div class="stat-sum-item"><div class="stat-sum-label">현재실적 합계</div><strong>${fK(tot.cur)}천원</strong></div>
        <div class="stat-sum-item"><div class="stat-sum-label">순증 합계</div><strong>${fK(tot.net)}천원</strong></div>
      ...
      <div class="stat-chart-section">
        <div class="stat-chart-title">지역단별 달성률 ${step === "2" ? "(Step 2 기준)" : "(Step 1 기준)"} — ${cohortLabel}순위</div>
  ...
  async function exportProgressAwardExcel() {
    ...
    const S_AMT_NUM  = (fill) => mkS(fill || "F0FDF4", "14532D", false, NUM_FMT, true);

    // 지점별 색상 팔레트 (최대 12개)
    const BRANCH_PALETTE = [
      "DBEAFE","D1FAE5","FEF9C3","FFE4E6","EDE9FE","FEE2E2",
      "CFFAFE","FEF3C7","DCFCE7","E0E7FF","FAFAFA","FDF4FF"
    ];
    const branchColorMap = {};
    let _bci = 0;
    const getBranchColor = (branch) => {
      if (!branch) return "F8FAFC";
      if (!branchColorMap[branch]) branchColorMap[branch] = BRANCH_PALETTE[_bci++ % BRANCH_PALETTE.length];
      return branchColorMap[branch];
    };

    // 셀 쓰기 헬퍼
    const ws_set = (ws, r, c, v, style) => {
      const addr = XLSX.utils.encode_cell({ r, c });
      const t = typeof v === "number" ? "n" : "s";
      ws[addr] = { v: v ?? "", t, s: style || mkS("FFFFFF") };
    };
    const ws_extend = (ws, r, c) => {
      if (!ws["!ref"]) { ws["!ref"] = XLSX.utils.encode_range({ s: { r, c }, e: { r, c } }); return; }
      const rng = XLSX.utils.decode_range(ws["!ref"]);
      if (r < rng.s.r) rng.s.r = r; if (c < rng.s.c) rng.s.c = c;
      if (r > rng.e.r) rng.e.r = r; if (c > rng.e.c) rng.e.c = c;
      ws["!ref"] = XLSX.utils.encode_range(rng);
    };
    const SC = (ws, r, c, v, style) => { ws_set(ws, r, c, v, style); ws_extend(ws, r, c); };

    // 물품 시상 원화 환산 ("주유권5만원" → 50000)
    const prizeKrw = (label) => {
      if (!label || label === "-") return 0;
      const m = String(label).match(/(\d+(?:\.\d+)?)\s*만/);
      if (m) return Math.floor(Number(m[1]) * 10000);
      const m2 = String(label).match(/(\d+(?:\.\d+)?)\s*원/);
    ...
    const getRateAward = (empNo) => {
      const rA = _bothAsgn.rateAsgn.get(empNo);
      if (!rA || rA.status !== "mine" || !_pa.rateConfig) return { label: "-", rank: "-", krw: 0 };
      const label = rankAwardLabel(_pa.rateConfig, rA.effectiveRank);
      return { label, rank: rA.effectiveRank, krw: prizeKrw(label) };
    };
    const getAmtAward = (empNo) => {
      const aA = _bothAsgn.amtAsgn.get(empNo);
      if (!aA || aA.status !== "mine" || !_pa.amtConfig) return { label: "-", rank: "-", krw: 0 };
      const label = rankAwardLabel(_pa.amtConfig, aA.effectiveRank);
      return { label, rank: aA.effectiveRank, krw: prizeKrw(label) };
    };

    const rateN = _pa.rateConfig ? Number(_pa.rateConfig.n) || 0 : 0;
    const amtN  = _pa.amtConfig  ? Number(_pa.amtConfig.n)  || 0 : 0;

    // ── 그룹시상 per-person 팀 시상금액 맵 (empNo → { label, krw }) ──
    const teamAwardMap = new Map();
    if (ga1En || ga2En) {
      groupRanking.forEach(g => {
        const mStats = g.memberStats || [];
        if (!mStats.length) return;
        let ga1Krw = 0, ga1Labels = [];
        if (ga1En) {
          const metGa1 = ga1ItemList.filter(it =>
            mStats.every(st => (st.net || 0) >= Number(it.threshold || 5) * 10000)
          );
          if (metGa1.length) {
            ga1Labels = metGa1.map(it => payoutLabel(it.payout));
            ga1Krw = ga1Labels.reduce((s, l) => s + prizeKrw(l), 0);
          }
        }
        let ga2Krw = 0, ga2Label = "";
        if (ga2En) {
          const ga1MetAny = ga1En ? ga1ItemList.some(it =>
            mStats.every(st => (st.net || 0) >= Number(it.threshold || 5) * 10000)
          ) : true;
          if (!ga1En || ga1MetAny) {
            ...
            if (metGa2.length) {
              ...
              ga2Label = payoutLabel(bestPay);
              ga2Krw = prizeKrw(ga2Label);
            }
          }
        }
        const totalKrw = ga1Krw + ga2Krw;
        const allLabel = [...ga1Labels, ga2Label].filter(Boolean).join("+") || "-";
        mStats.forEach(st => {
          if (st.s?.empNo) teamAwardMap.set(st.s.empNo, { label: allLabel, krw: totalKrw });
        });
      });
    }

    // ── 비전센터별 시상 데이터 수집 (centerAwardMap) ──
    const statsMap = new Map();
    byAmt.forEach(st => statsMap.set(st.s.empNo, { base: st.base || 0, current: st.current || 0, net: st.net || 0, rate: st.rate || 0 }));

    const centerAwardMap = new Map();
    const ensurePerson = (empNo, s, fallbackNet) => {
      if (!centerAwardMap.has(empNo)) {
        const ps = statsMap.get(empNo) || { base: 0, current: 0, net: fallbackNet || 0, rate: 0 };
        centerAwardMap.set(empNo, {
          empNo, name: s.name || "", center: s.center || "", branch: s.branch || "",
          base: ps.base, current: ps.current, net: ps.net, rate: ps.rate,
          piLabel: "-", piKrw: 0,
          rateLabel: "-", rateRank: "-",
          amtLabel: "-", amtRank: "-",
          teamLabel: "-", teamKrw: 0,
      ...
      return centerAwardMap.get(empNo);
    };
    byAmt.forEach(st => {
      const _tlbl = tierLabel(st.net, undefined, _pa);
      if (_tlbl === "-") return;
      const _tamt = tierAward(st.net, undefined, _pa);
      const p = ensurePerson(st.s.empNo, st.s);
      p.piLabel = _tlbl;
      p.piKrw = _tamt > 0 ? Math.floor(_tamt) : prizeKrw(_tlbl);
    });
    byRate.forEach(st => {
      const rA = _bothAsgn.rateAsgn.get(st.s.empNo);
      if (rA && rA.status === "mine") {
        const p = ensurePerson(st.s.empNo, st.s);
        p.rateLabel = rankAwardLabel(_pa.rateConfig, rA.effectiveRank);
        p.rateRank = rA.effectiveRank;
      }
    });
    byAmt.forEach(st => {
      const aA = _bothAsgn.amtAsgn.get(st.s.empNo);
      if (aA && aA.status === "mine") {
        const p = ensurePerson(st.s.empNo, st.s);
        p.amtLabel = rankAwardLabel(_pa.amtConfig, aA.effectiveRank);
        p.amtRank = aA.effectiveRank;
      }
    });
    teamAwardMap.forEach((award, empNo) => {
      if (award.krw <= 0) return;
      let foundS = null, foundNet = 0;
      for (const g of groupRanking) {
        const ms = (g.memberStats || []).find(st => st.s?.empNo === empNo);
        if (ms) { foundS = ms.s; foundNet = ms.net || 0; break; }
      }
      if (!foundS) return;
      const p = ensurePerson(empNo, foundS, foundNet);
      p.teamLabel = award.label;
      p.teamKrw = award.krw;
    });
    ...
    // ── 수상내역 시트 ──
    {
      ...
      SC(ws, R, 0, `${baseTitle} 수상내역`, S_TITLE);
      ws["!merges"].push({ s: { r: R, c: 0 }, e: { r: R, c: TOTAL_COLS - 1 } });
      R++;

      // ── 개인순증시상 ──
      if (_pa.tiers.length) {
        R++;
        SC(ws, R, 0, "▶ 개인순증시상", S_SEC);
        ws["!merges"].push({ s: { r: R, c: 0 }, e: { r: R, c: TOTAL_COLS - 1 } });
        R++;
        const piHdr = ["순번","사번","성명","비전센터","지점","기준실적(원)","현재실적(원)","순증(원)","달성률(%)","시상내역","시상금액(원)","신장률시상","신장률순위","신장액시상","신장액순위"];
        ...
        if (!piCnt) {
          SC(ws, R, 0, "", S_TXT("F8FAFC")); SC(ws, R, 1, "(해당자 없음)", S_TXT("F8FAFC"));
          ...
          ws["!merges"].push({ s: { r: R, c: 11 }, e: { r: R, c: TOTAL_COLS - 1 } });
          R++;
        }
      }

      // ── 신장률 TOP + 신장액 TOP (좌우 병렬) ──
      if (rateN > 0 || amtN > 0) {
        R++;
        SC(ws, R, 0, `📈 신장률 TOP${rateN}`, S_RATE_HDR);
        ws["!merges"].push({ s: { r: R, c: 0 }, e: { r: R, c: 4 } });
        SC(ws, R, 6, `💰 신장액 TOP${amtN}`, S_AMT_HDR);
        ws["!merges"].push({ s: { r: R, c: 6 }, e: { r: R, c: 10 } });
        R++;
        ["순위","성명","달성률(%)","순증(원)","시상"].forEach((h, c) => SC(ws, R, c, h, S_HDR));
        SC(ws, R, 5, "", mkS("F8FAFC"));
        ["순위","성명","순증(원)","달성률(%)","시상"].forEach((h, c) => SC(ws, R, 6 + c, h, S_HDR));
        R++;

        let rateCnt = 0, amtCnt = 0, rateTotalKrw = 0, amtTotalKrw = 0;
        const maxN = Math.max(rateN, amtN, 1);
        for (let rank = 1; rank <= maxN; rank++) {
          const re = _rateWinMap.get(rank);
          if (re && _pa.rateConfig && rank <= rateN) {
            const { st, rA } = re;
            const lbl = rankAwardLabel(_pa.rateConfig, rA.effectiveRank);
            SC(ws, R, 0, rank,                           S_RATE_ROW());
            SC(ws, R, 1, st.s.name || "",                S_RATE_ROW());
            SC(ws, R, 2, parseFloat(st.rate.toFixed(1)), S_RATE_ROW());
            SC(ws, R, 3, st.net,                          S_RATE_NUM());
            SC(ws, R, 4, lbl,                             S_RATE_ROW());
            rateTotalKrw += prizeKrw(lbl); rateCnt++;
          } else if (rank <= rateN) {
            SC(ws, R, 0, rank, S_RATE_ROW()); SC(ws, R, 1, "-", S_RATE_ROW());
            SC(ws, R, 2, "-",  S_RATE_ROW()); SC(ws, R, 3, "-", S_RATE_ROW()); SC(ws, R, 4, "-", S_RATE_ROW());
          }
          SC(ws, R, 5, "", mkS("F8FAFC"));
          const ae = _amtWinMap.get(rank);
          if (ae && _pa.amtConfig && rank <= amtN) {
            const { st, aA } = ae;
            const lbl = rankAwardLabel(_pa.amtConfig, aA.effectiveRank);
            SC(ws, R, 6,  rank,                           S_AMT_ROW());
            SC(ws, R, 7,  st.s.name || "",                S_AMT_ROW());
            SC(ws, R, 8,  st.net,                          S_AMT_NUM());
            SC(ws, R, 9,  parseFloat(st.rate.toFixed(1)), S_AMT_ROW());
            SC(ws, R, 10, lbl,                             S_AMT_ROW());
            amtTotalKrw += prizeKrw(lbl); amtCnt++;
          } else if (rank <= amtN) {
            SC(ws, R, 6, rank, S_AMT_ROW()); SC(ws, R, 7,  "-", S_AMT_ROW());
            SC(ws, R, 8, "-",  S_AMT_ROW()); SC(ws, R, 9,  "-", S_AMT_ROW()); SC(ws, R, 10, "-", S_AMT_ROW());
          }
          if (rank <= rateN || rank <= amtN) R++;
        }
        // 합계 행
        SC(ws, R, 0, `신장률 ${rateCnt}명`, S_RATE_HDR);
        ws["!merges"].push({ s: { r: R, c: 0 }, e: { r: R, c: 3 } });
        SC(ws, R, 4, rateTotalKrw > 0 ? rateTotalKrw : "", rateTotalKrw > 0 ? S_RATE_TOT : S_RATE_HDR);
        SC(ws, R, 5, "", mkS("F8FAFC"));
        SC(ws, R, 6, `신장액 ${amtCnt}명`, S_AMT_HDR);
        ws["!merges"].push({ s: { r: R, c: 6 }, e: { r: R, c: 9 } });
        SC(ws, R, 10, amtTotalKrw > 0 ? amtTotalKrw : "", amtTotalKrw > 0 ? S_AMT_TOT : S_AMT_HDR);
        R++;
      }
      ...
      if (ga1En || ga2En) {
        R++;
        SC(ws, R, 0, `▶ 그룹시상 (${groupLabel}별)`, S_SEC);
        ws["!merges"].push({ s: { r: R, c: 0 }, e: { r: R, c: TOTAL_COLS - 1 } });
        R++;
        // 미달금액 제거, 순증(원)/달성률(%)/시상금액(원) 추가
        const grpHdr = ["순위", groupLabel, "기준합계(원)", "현재합계(원)", "달성률(%)"];
        if (ga1En) grpHdr.push("그룹시상1");
        if (ga2En) grpHdr.push("그룹시상2");
        grpHdr.push("구성원", "달성여부", "순증(원)", "달성률(%)", "시상금액(원)");
        grpHdr.forEach((h, c) => SC(ws, R, c, h, S_HDR));
        R++;

        const ga1Thr  = ga1En && ga1ItemList.length ? Number(ga1ItemList[0].threshold || 5) * 10000 : 0;
        const memberC = grpHdr.length - 5;
        const statusC = grpHdr.length - 4;
        const netC    = grpHdr.length - 3;
        ...
        groupRanking.forEach((g, i) => {
          ...
          if (ga1En) {
            const met = ga1ItemList.filter(it =>
              mStats.every(st => (st.net || 0) >= Number(it.threshold || 5) * 10000) && mStats.length > 0
            );
            gRow.push(met.length ? met.map(it => payoutLabel(it.payout)).join("+") : "미달");
          }
          if (ga2En) {
            const ga1Met = ga1En && ga1ItemList.some(it =>
              mStats.every(st => (st.net || 0) >= Number(it.threshold || 5) * 10000) && mStats.length > 0
            );
            if (ga1En && !ga1Met) { gRow.push("미달(그룹1↑필요)"); }
            else {
              const metGa2 = ga2ItemList.filter(it => g.rate >= Number(it.rateThreshold || 110));
              if (metGa2.length) {
                const bestPay = metGa2.reduce((best, it) => {
                ...
                gRow.push(payoutLabel(bestPay));
              } else { gRow.push(`미달(${ga2ItemList[0]?.rateThreshold || 110}%↑)`); }
            }
          }
          ...
          // 구성원 — 첫 번째 멤버를 groupStartRow와 같은 행에 배치
          if (!mStats.length) {
            SC(ws, groupStartRow, memberC, "", S_TXT("F8FAFC"));
            SC(ws, groupStartRow, statusC, "", S_TXT("F8FAFC"));
            SC(ws, groupStartRow, netC,    "", S_TXT("F8FAFC"));
            SC(ws, groupStartRow, mRateC,  "", S_TXT("F8FAFC"));
            SC(ws, groupStartRow, awardC,  "", S_TXT("F8FAFC"));
            R = groupStartRow + 1;
          } else {
            mStats.forEach((st, mi) => {
              const rowR = groupStartRow + mi;
              const mNet = st.net || 0;
              const ach  = ga1Thr > 0 ? mNet >= ga1Thr : true;
              const mRateVal = ga1Thr > 0
                ? parseFloat((mNet / ga1Thr * 100).toFixed(1))
                : (typeof st.rate === "number" ? parseFloat(st.rate.toFixed(1)) : 0);
              const mAward = teamAwardMap.get(st.s?.empNo);
              ...
              SC(ws, rowR, awardC,  mAwardKrw > 0 ? mAwardKrw : "", mAwardKrw > 0 ? numS : nameS);
            });
    ...
    // ── 전체검증 시트 ──
    {
      ...
      SC(ws, R, 0, `${baseTitle} 전체 교육생 시상 검증`, S_TITLE);
      ws["!merges"].push({ s: { r: R, c: 0 }, e: { r: R, c: VCOLS - 1 } });
      R += 2;

      const vHdr = ["순번","사번","성명","비전센터","지점","기준(원)","현재(원)","순증(원)","달성률(%)","개인순증시상","시상금액(원)","신장률TOP","신장액TOP"];
      vHdr.forEach((h, c) => SC(ws, R, c, h, S_HDR));
      R++;

      byAmt.forEach((st, i) => {
        const _tamt = tierAward(st.net, undefined, _pa);
        const _tlbl = tierLabel(st.net, undefined, _pa);
        const tierText = _tlbl !== "-" ? _tlbl : (_pa.tiers.length > 0 ? "해당없음" : "-");
        const rA = _bothAsgn.rateAsgn.get(st.s.empNo);
        const aA = _bothAsgn.amtAsgn.get(st.s.empNo);
        const rateTxt = !rA ? "-"
        ...
        SC(ws, R, 7,  st.net,                             S_NUM(bc));
        SC(ws, R, 8,  parseFloat(st.rate.toFixed(1)),    S_TXT(bc));
        SC(ws, R, 9,  tierText,                           S_TXT(bc));
        SC(ws, R, 10, _tamt > 0 ? Math.floor(_tamt) : "", _tamt > 0 ? S_NUM(bc) : S_TXT(bc));
        SC(ws, R, 11, rateTxt,                            S_TXT(bc));
        SC(ws, R, 12, amtTxt,                             S_TXT(bc));
        R++;
      });

      ws["!cols"] = [
        { wch: 5 }, { wch: 11 }, { wch: 10 }, { wch: 16 }, { wch: 14 },
        { wch: 14 }, { wch: 14 }, { wch: 14 }, { wch: 9 },
        { wch: 16 }, { wch: 14 }, { wch: 20 }, { wch: 20 },
      ];
      XLSX.utils.book_append_sheet(wb, ws, "전체검증");
    }

    // ── 비전센터별 시트 ──
    {
      const centerGroups = new Map();
      centerAwardMap.forEach(rec => {
        const k = rec.center || "미지정";
        if (!centerGroups.has(k)) centerGroups.set(k, []);
        centerGroups.get(k).push(rec);
      });
      const C_HDR = ["사번","성명","지점","기준실적(원)","현재실적(원)","순증(원)","달성률(%)","개인시상","개인금액(원)","신장률시상","신장률순위","신장액시상","신장액순위","팀시상","팀시상금액(원)"];
      centerGroups.forEach((recs, cName) => {
        recs.sort((a, b) => {
          const bc = (a.branch || "").localeCompare(b.branch || "");
          return bc !== 0 ? bc : (b.net || 0) - (a.net || 0);
        });
        const ws = {};
        ws["!merges"] = [];
        let R = 0;
        ...
        R++;
        let totalPiKrw = 0, totalTeamKrw = 0, cnt = 0;
        recs.forEach(rec => {
          const bc = getBranchColor(rec.branch);
          const mRate = rec.rate > 0 ? parseFloat(rec.rate.toFixed(1)) :
                        (rec.base > 0 ? parseFloat((rec.current / rec.base * 100).toFixed(1)) : 0);
          SC(ws, R, 0,  rec.empNo || "",                              S_TXT(bc));
          SC(ws, R, 1,  rec.name || "",                               S_TXT(bc));
          SC(ws, R, 2,  rec.branch || "",                             S_TXT(bc));
          SC(ws, R, 3,  rec.base > 0 ? rec.base : "",                 rec.base > 0 ? S_NUM(bc) : S_TXT(bc));
          SC(ws, R, 4,  rec.current > 0 ? rec.current : "",           rec.current > 0 ? S_NUM(bc) : S_TXT(bc));
          SC(ws, R, 5,  rec.net > 0 ? rec.net : "",                   rec.net > 0 ? S_NUM(bc) : S_TXT(bc));
          SC(ws, R, 6,  mRate > 0 ? mRate : "",                       S_TXT(bc));
          SC(ws, R, 7,  rec.piLabel !== "-" ? rec.piLabel : "",       S_TXT(bc));
          SC(ws, R, 8,  rec.piKrw > 0 ? rec.piKrw : "",              rec.piKrw > 0 ? S_NUM(bc) : S_TXT(bc));
          SC(ws, R, 9,  rec.rateLabel !== "-" ? rec.rateLabel : "",   S_TXT(bc));
          SC(ws, R, 10, rec.rateRank !== "-" ? rec.rateRank : "",     S_TXT(bc));
          SC(ws, R, 11, rec.amtLabel !== "-" ? rec.amtLabel : "",     S_TXT(bc));
          SC(ws, R, 12, rec.amtRank !== "-" ? rec.amtRank : "",       S_TXT(bc));
          SC(ws, R, 13, rec.teamLabel !== "-" ? rec.teamLabel : "",   S_TXT(bc));
          SC(ws, R, 14, rec.teamKrw > 0 ? rec.teamKrw : "",          rec.teamKrw > 0 ? S_NUM(bc) : S_TXT(bc));
          totalPiKrw += rec.piKrw || 0;
          totalTeamKrw += rec.teamKrw || 0;
          cnt++; R++;
        });
        // 합계 행
        SC(ws, R, 0, `총 ${cnt}명`, S_TOTAL_TXT);
        ws["!merges"].push({ s: { r: R, c: 0 }, e: { r: R, c: 7 } });
        SC(ws, R, 8, totalPiKrw > 0 ? totalPiKrw : "", totalPiKrw > 0 ? S_TOTAL : S_TOTAL_TXT);
        SC(ws, R, 9, "", S_TOTAL_TXT);
        ws["!merges"].push({ s: { r: R, c: 9 }, e: { r: R, c: 13 } });
        SC(ws, R, 14, totalTeamKrw > 0 ? totalTeamKrw : "", totalTeamKrw > 0 ? S_TOTAL : S_TOTAL_TXT);
        R++;
        ws["!cols"] = [
          { wch: 11 }, { wch: 10 }, { wch: 14 }, { wch: 14 }, { wch: 14 },
          { wch: 14 }, { wch: 9 },  { wch: 16 }, { wch: 14 },
          { wch: 14 }, { wch: 9 },  { wch: 14 }, { wch: 9 }, { wch: 20 }, { wch: 14 },
        ];
        const safeName = String(cName).replace(/[\\/:\*\?\[\]]/g, "").trim().substring(0, 31) || "기타센터";
  ...
  function _apNpMigrateItem(item) {
    if (item.conditions) return item;
    const conditions = [];
    if (item.cond1Type === "text") {
      conditions.push({ enabled: true, type: "text", text: item.cond1Text || "" });
    } else {
      conditions.push({ enabled: true, type: "amount", amount: item.cond1Amount || 0 });
    }
    if (item.cond2) {
      conditions.push({ enabled: true, type: "text", text: item.cond2 });
    }
    if (item.paySlabs?.length || item.payType) {
      conditions.push({ enabled: true, type: "slab", payType: item.payType || "pct", slabs: item.paySlabs?.length ? item.paySlabs : [{ threshold: 0, value: 0 }] });
    }
    return { productName: item.productName || "", conditions };
  }

  function _apRenderNpCond(cond, ci) {
    const type = cond.type || "amount";
    const enabled = cond.enabled !== false;
    const payType = cond.payType || "pct";
    const slabs = cond.slabs?.length ? cond.slabs : [{ threshold: 0, value: 0 }];
    ...
    const typeLabel = type === "amount" ? "금액" : type === "text" ? "텍스트" : "구간";
    const slabBlockDisplay = type === "slab" ? "display:flex;flex-direction:column;gap:4px;" : "display:none;";
    return `<div class="ap-np-cond" data-ci="${ci}" style="opacity:${enabled ? "1" : "0.55"}">
      <div class="ap-np-row" style="gap:4px;flex-wrap:wrap;">
        <input type="checkbox" class="ap-np-cond-en" ${enabled ? "checked" : ""} title="이 조건 사용">
        <button type="button" class="ap-toggle-btn ap-np-cond-type" data-val="${type}">${typeLabel}</button>
        <input type="number" class="pg-input ap-np-cond-amount-val" value="${cond.amount || 0}" min="0" step="1" style="width:80px;${type !== "amount" ? "display:none;" : ""}">
        <span class="ap-np-cond-amount-unit" style="font-size:12px;${type !== "amount" ? "display:none;" : ""}">만원 이상</span>
        <input type="text" class="pg-input ap-np-cond-text-val" value="${escapeHtml(cond.text || "")}" placeholder="예: 5~10일, 통합사망 3천만원 이상" style="width:230px;${type !== "text" ? "display:none;" : ""}">
        <button type="button" class="ap-toggle-btn ap-np-paytype" data-val="${payType}" style="${type !== "slab" ? "display:none;" : ""}">${payType === "pct" ? "%" : "정액"}</button>
        <button type="button" class="ap-del-btn ap-np-cond-del" title="조건삭제" style="margin-left:auto;">✕</button>
      </div>
      <div class="ap-np-slabs-block" style="padding-left:54px;margin-top:4px;${slabBlockDisplay}">
  ...
  function _apRenderNp(items) {
    ...
    el.innerHTML = items.map((item, i) => {
      const migrated = _apNpMigrateItem(item);
      const conds = migrated.conditions || [];
      const condsHtml = conds.map((c, ci) => _apRenderNpCond(c, ci)).join("");
      return `
        <div class="ap-np-card" data-i="${i}">
          ...
          <div class="ap-np-row">
            ...
            <input type="text" class="pg-input ap-np-name" value="${escapeHtml(migrated.productName || "")}" placeholder="상품명 입력" style="width:200px;">
          </div>
          <div class="ap-np-conds" style="margin-top:6px;display:flex;flex-direction:column;gap:6px;">
            ${condsHtml}
          </div>
          <button type="button" class="btn-outline small ap-np-cond-add" style="width:fit-content;margin-top:6px;">＋ 조건추가</button>
        </div>`;
    }).join("");
  }
  ...
  function _apCollectNp() {
    ...
    document.querySelectorAll("#ap-np-list .ap-np-card").forEach((card) => {
      const conditions = [];
      card.querySelectorAll(".ap-np-cond").forEach((condEl) => {
        const type = condEl.querySelector(".ap-np-cond-type")?.dataset.val || "amount";
        const enabled = condEl.querySelector(".ap-np-cond-en")?.checked !== false;
        const cond = { enabled, type };
        if (type === "amount") {
          cond.amount = Number(condEl.querySelector(".ap-np-cond-amount-val")?.value) || 0;
        } else if (type === "text") {
          cond.text = condEl.querySelector(".ap-np-cond-text-val")?.value.trim() || "";
        } else if (type === "slab") {
          const payType = condEl.querySelector(".ap-np-paytype")?.dataset.val || "pct";
          const slabs = [];
          condEl.querySelectorAll(".ap-np-slab").forEach((s) => {
            slabs.push({
          ...
          cond.slabs = slabs.length ? slabs : [{ threshold: 0, value: 0 }];
        }
  ...
  function bindAwardPlanModal() {
    ...
    modal.addEventListener("click", (e) => {
      ...
      if (tg) {
        if (tg.id === "ap-t1-type" || tg.id === "ap-t2-type") {
          ...
          const cur = tg.dataset.val;
          const next = cur === "amount" ? "text" : cur === "text" ? "slab" : "amount";
          const nextLabel = next === "amount" ? "금액" : next === "text" ? "텍스트" : "구간";
          tg.dataset.val = next;
          tg.textContent = nextLabel;
          const condEl = tg.closest(".ap-np-cond");
          if (condEl) {
            condEl.querySelector(".ap-np-cond-amount-val").style.display = next === "amount" ? "" : "none";
            condEl.querySelector(".ap-np-cond-amount-unit").style.display = next === "amount" ? "" : "none";
            condEl.querySelector(".ap-np-cond-text-val").style.display = next === "text" ? "" : "none";
            condEl.querySelector(".ap-np-paytype").style.display = next === "slab" ? "" : "none";
            const slabBlock = condEl.querySelector(".ap-np-slabs-block");
            if (slabBlock) slabBlock.style.display = next === "slab" ? "flex" : "none";
            if (next === "slab") slabBlock.style.flexDirection = "column";
            if (next === "slab") slabBlock.style.gap = "4px";
          }
      ...
      // 신상품 조건추가
      const npCondAdd = e.target.closest(".ap-np-cond-add");
      if (npCondAdd) {
        const card = npCondAdd.closest(".ap-np-card");
        const condsEl = card?.querySelector(".ap-np-conds");
        if (condsEl) {
          const ci = condsEl.querySelectorAll(".ap-np-cond").length;
          condsEl.insertAdjacentHTML("beforeend", _apRenderNpCond({ enabled: true, type: "amount", amount: 0 }, ci));
        }
        return;
      }
      // 신상품 구간추가
      ...
      if (npSlabAdd) {
        const condEl   = npSlabAdd.closest(".ap-np-cond");
        const slabsEl  = condEl?.querySelector(".ap-np-slabs");
        const payType  = condEl?.querySelector(".ap-np-paytype")?.dataset.val || "pct";
        if (slabsEl) {
    ...
    document.getElementById("ap-np-add")?.addEventListener("click", () => {
      ...
      cur.push({ productName: "", conditions: [{ enabled: true, type: "amount", amount: 0 }] });
      _apRenderNp(cur);
    });
